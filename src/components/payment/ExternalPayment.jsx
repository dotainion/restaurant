...
export const ExternalPayment = () => {
    ...
    const submit = (e) =>{
        ...
        setLoading(true);
        const data = {
            posReference: posIdRef.current.value,
    ...
    return (
        <form onSubmit={submit} className="position-relative">
            ...
            <div className="border-lightly form-control bg-dark border shadow-none text-light mb-3">
                {service.type}
            </div>
            <TableNumberInput/>
            <div className="small mb-1">POS Reference #</div>
            <input
            ...
            <input
                ref={amountRef}
                className="border-lightly form-control bg-dark border shadow-none text-light mb-3"
                placeholder="Enter amount confirmed on POS..."
                type="number"
                defaultValue={total}
                readOnly 
                required
            ...
            <div className="small text-warning mb-4">
                Please confirm that the payment was successfully processed on the external POS before completing the order.
            </div>
            <div className="d-flex gap-2 text-nowrap mt-4">
                <button
                    onClick={paymentUtils.hide}
                    className="rounded-start-3 btn btn-outline-orange rounded-0 border border-orange w-100"
                >Cancel</button>
